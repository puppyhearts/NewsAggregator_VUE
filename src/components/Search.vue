	...
	<transition name="slide-fade">
		<div v-if="!filterHidden" class="w-full text-center md:mx-6 pb-4 md:mb-3 z-0">
			...
			<div class="block md:inline mt-2 md:mt-0 md:ml-6">
				...
				<select class="inline w-full max-w-xs md:max-w-max md:w-auto border-transparent bg-gray-200 dark:bg-gray-900 text-black dark:text-white text-sm p-2 ml-1 md:ml-2 rounded-full" v-model="options.countries">
					<option disabled value="">Countries</option>
					<option class="text-sm" v-for="countries in country_options" :key="countries" :value="Countries">
						{{ countries }} 
					</option>
				</select>
			</div>
...
export default {
	...
	setup(props, context) {
		const lang_options = [
			{ text: 'English (All variants)', value: 'en' },
			{ text: 'ไทย', value: 'th' },
